...
class EpuckForagingSupervisor(DeepbotsSupervisorEnv):
    ...
    def step(self, action):
        ...
        for i in range(self.num_robots):
            ...
            # Max Speed Logic: PPO outputs [-1, 1]. 
            # We map this to max motor speed. E-puck max is ~6.28 rad/s.
            # Let's assume driver handles scaling or we send raw [-1, 1] and driver scales.
            # Driver expects [-1, 1] * MAX_SPEED.
    ...
    def get_observations(self):
        ...
        for i in range(self.num_robots):
            ...
            if not self.carrying_state[i]:
                for tag_node in self.tag_nodes:
                    ...
                    if dist < 2.0: # 2m Range
                        ...
                        if angle < 1.0: # ~60 deg FOV (wider for better detection)
                            if dist < min_dist:
                                min_dist = dist
            ...
            # D. Pheromone Sensors
            # Sample grid at 3 points: Front, Left, Right
            # Robot pos in grid coords
            gx = int((robot_pos[0] + 2.5) / self.grid_res)
            gy = int((robot_pos[1] + 2.5) / self.grid_res)
            
            # Helper to get grid value safely
            ...
            # Left/Right offsets (rotate forward vec 45 deg)
            # Left
            ...
            # [prox(8), tag_visible, tag_dist, tag_angle, carrying, phero_front, phero_left, phero_right]
            obs = []
            obs.extend(prox)
    ...
    def get_reward(self, action):
        ...
        for i in range(self.num_robots):
            ...
            # 1. Logic: Pick Up
            if not self.carrying_state[i]:
                # Check distance to all tags
                for tag_node in self.tag_nodes:
                    tag_pos = tag_node.getPosition()
                    if tag_pos[2] < 0: continue # Already taken
                    
                    dx = tag_pos[0] - robot_pos[0]
                    dy = tag_pos[1] - robot_pos[1]
                    dist = math.sqrt(dx*dx + dy*dy)
                    
                    if dist < 0.15: # Pick Up Threshold
                        self.carrying_state[i] = True
                        # Hide Tag (Move far away)
                        # Ideally we attach it to robot, but hiding is simpler for logic
                        # We can visualize it by moving it to robot z+0.05
                        tag_node.getField("translation").setSFVec3f([0, 0, -10]) # Hide
                        total_reward += 1.0 # Pick Up Reward
                        print(f"Robot {i+1} picked up a tag!")
                        break
            ...
            # 2. Logic: Deposit
                ...
                if dist_to_base < 0.3: # Base Radius + Robot Radius
                    self.carrying_state[i] = False
                    total_reward += 10.0 # Deposit Reward
                    print(f"Robot {i+1} deposited food!")
                    
                    # Respawn a tag (find a hidden one)
                    for tag_node in self.tag_nodes:
                        if tag_node.getPosition()[2] < 0:
                            # Respawn in a random cluster
                            # Simple logic: Random pos in arena
            ...
            # If Searching: Move to Tag (if visible)
            # Logic handled implicitly by "tag_dist" observation + PPO learning
...
if __name__ == "__main__":
    ...
    model_path = "ppo_epuck_heavy"
    
    # HEAVY Training Config (Optimized for GPU)
    policy_kwargs = dict(
        activation_fn=torch.nn.ReLU,
        net_arch=[512, 512, 512]  # Very large network
    )
    
    if os.path.exists(model_path + ".zip"):
        print("Loading existing heavy model...")
        model = PPO.load(model_path, env=env)
    else:
        print("Creating new HEAVY model...")
        print("Network: [512, 512, 512] neurons")
        print("Batch Size: 4096 (GPU optimized)")
        model = PPO(
            "MlpPolicy", 
            env, 
            verbose=1, 
            tensorboard_log="./ppo_heavy_tensorboard/",
            policy_kwargs=policy_kwargs,
            n_steps=4096,
            batch_size=4096,  # Increased for GPU
            ent_coef=0.01,
            learning_rate=3e-4,
    ...
    print("\n" + "="*60)
    print("STARTING HEAVY GPU TRAINING")
    print("="*60)
    TIMESTEPS = 100000  # Reduced for faster iteration
    print(f"Total Timesteps: {TIMESTEPS:,}")
    print(f"Estimated Updates: {TIMESTEPS // 4096}")
    print("="*60 + "\n")
