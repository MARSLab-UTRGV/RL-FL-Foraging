...
# =============================================================================
# FULLY DECENTRALIZED TRAINING ROBOT — v11
#
# Changes from v8:
#   1. Near-base crowding penalty: when robot is within 1.5m of base AND
#      detects a nearby object via prox (> 0.1), apply extra -max_prox * 0.5
#      penalty. Approximates the inter-robot separation penalty in centralized
#      supervisor (supervisor_cpfa.py lines 642-654) using only onboard sensors.
#      Per-robot reward has no cross-robot GPS access; prox near base is a
#      strong proxy for nest clustering.
#
# All other hyperparameters, obs space (19D), pheromone, and gossip unchanged.
...
class EpuckDecentralizedTrainV11(EpuckDecentralizedV4):
    ...
    def __init__(self):
        ...
        self._run_name = open(cfg).read().strip() if os.path.exists(cfg) else 'decentralized_indep_v11'
    ...
    def _compute_reward(self, obs_comp, pickup_signal):
        ...
        # Near-base crowding penalty — mirrors centralized inter-robot separation
        # penalty (supervisor_cpfa.py lines 642-654). Centralized penalises -0.5*(1-sep)
        # per pair when both robots are within 1.5m of base. Here we approximate with
        # prox (can't read cross-robot GPS per-robot), adding an extra -max_prox*0.5
        # whenever the robot is in the nest zone (dist < 1.5m) and detects a nearby
        # object. Near the nest walls are far away, so high prox signals nearby robots.
        dist_raw = math.sqrt(gps_x ** 2 + gps_y ** 2)
        if dist_raw < 1.5 and max_prox > 0.1:
            reward -= max_prox * 0.5
...
if __name__ == '__main__':
    robot = EpuckDecentralizedTrainV11()
    robot.run()
