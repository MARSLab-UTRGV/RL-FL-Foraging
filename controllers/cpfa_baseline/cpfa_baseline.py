...
class CPFABaseline(Supervisor):
    def __init__(self, params_path):
        ...
        self.step_count     = 0
        self.eighty_percent_deposits = 51
    ...
    def _new_results_path(self):
        ...
        while os.path.exists(path):
            path = os.path.join(out_dir,
                                f"cpfa_baseline_results_{stamp}_{suffix}.txt")
            suffix += 1
        return path
    ...
    def _record_eighty_percent_if_needed(self):
        ...
        msg = (
            f"\n{'='*70}\n"
            f"80% COMPLETE: {self.eighty_percent_deposits}/64 deposits\n"
    ...
    def _final_summary(self):
        ...
        if self.eighty_percent_step is None:
            ...
            eighty_msg = (
                f"80% completion: {self.eighty_percent_deposits}/64 deposits at "
                f"step {self.eighty_percent_step} ({eighty_min:.2f} min), "
                f"rate {eighty_rate:.3f} tags/min\n"
            )
        return (
            f"\n{'='*70}\n"
            "COMPLETE: all 64 tags deposited\n"
            f"{eighty_msg}"
