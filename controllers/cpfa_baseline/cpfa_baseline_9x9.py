...
#
# Run:
#   webots worlds/eval_sample1_9x9.wbt
#   python3 controllers/cpfa_baseline/cpfa_baseline_9x9.py
# =============================================================================
...
class CPFABaseline(Supervisor):
    ...
    def _random_wall_target(self):
        ...
        r      = random.random()
        lo, hi = -4.415, 4.415   # ±(4.5m - 0.085m robot radius)
        if r < 0.25:
    ...
    def _crw_step(self, i, robot_pos, informed=False):
        ...
        step_dist = 0.08  # metres — ARGoS SearchStepSize=0.08m scaled to 9×9m arena
        ...
        tx = max(-4.2, min(4.2, tx))
        ty = max(-4.2, min(4.2, ty))
        return (tx, ty)
