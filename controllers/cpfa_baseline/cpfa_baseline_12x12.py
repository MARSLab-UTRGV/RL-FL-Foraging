...
#
# Run:
#   webots worlds/eval_sample1_12x12.wbt
#   python3 controllers/cpfa_baseline/cpfa_baseline_12x12.py
# =============================================================================
...
class CPFABaseline(Supervisor):
    def __init__(self):
        ...
        print("=" * 65)
        print("CPFA BASELINE — ARGoS-calibrated parameters  (12×12m arena  370 tags)")
    ...
    def _random_wall_target(self):
        ...
        r      = random.random()
        lo, hi = -5.915, 5.915   # ±(6.0m - 0.085m robot radius)
        if r < 0.25:
    ...
    def _crw_step(self, i, robot_pos, informed=False):
        ...
        step_dist = 0.08  # metres — ARGoS SearchStepSize=0.08m scaled to 12×12m arena
        ...
        tx = max(-5.7, min(5.7, tx))
        ty = max(-5.7, min(5.7, ty))
        return (tx, ty)
    ...
    def run(self):
        while self.step(self.timestep) != -1:
            ...
            for i in range(self.num_robots):
                ...
                wall_dist = 6.0 - max(abs(robot_pos[0]), abs(robot_pos[1]))
            ...
            if self.step_count % 500 == 0:
                ...
                for ri in range(self.num_robots):
                    ...
                    wall_d    = 6.0 - max(abs(rpos[0]), abs(rpos[1]))
                ...
                print(log_msg)
                with open("cpfa_baseline_log_12x12.txt", "a") as f:
                    f.write(log_msg)
