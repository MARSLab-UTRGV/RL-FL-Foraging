...
# =============================================================================
# EXPERIMENT 4b — Robot Fault Tolerance (5×5m arena)
#
# Identical to eval_best_model_5x5.py except one robot (--fault-robot 0..3)
# is permanently stopped from t=0.  The PPO network still receives all 4
# robot observations and produces 4 actions, but the faulty robot's action
# is overridden with [0, 0] every step.  The faulty robot is still present
# in the arena (physically blocking its initial position) and its sensor
# data is still processed, but it never moves.
#
# The runner (run_foraging_experiments.py) picks the faulty robot randomly
# per sample using a seeded RNG:
#   fault_robot = random.Random(sample * 97 + 41).randint(0, 3)
#
# Usage:
#   python eval_best_model_exp4b_5x5.py ppo_cpfa_v9 \
#       --fault-robot 2 --duration-sim-min 10.0
# =============================================================================
...
class EpuckForagingSupervisor(Supervisor, gym.Env):
    def __init__(self):
        ...
        # Set by main loop before first step
        self.fault_robot      = -1          # 0-indexed; -1 = no fault
        self.fault_active     = False       # becomes True when failure_time_min is reached
        self.failure_time_min = float('inf')
    ...
    def step(self, action):
        ...
        if (not self.fault_active and self.fault_robot >= 0
                and elapsed_min >= self.failure_time_min):
            self.fault_active = True
            print(f"[FAULT] Robot {self.fault_robot + 1} failed at "
                  f"t={elapsed_min:.3f} sim-min", flush=True)

        final_action = self._apply_overrides(action)

        # Zero the faulty robot's velocity once fault is active
        if self.fault_active:
            final_action[self.fault_robot * 2]     = 0.0
            final_action[self.fault_robot * 2 + 1] = 0.0
    ...
    def _process_events(self):
        ...
        for i in range(self.num_robots):
            if i == self.fault_robot and self.fault_active:
                continue    # faulty robot never picks up or deposits after failure
    ...
    def _apply_overrides(self, action):
        ...
        for i in range(self.num_robots):
            if i == self.fault_robot and self.fault_active:
                continue    # faulty robot handled separately once fault is active
            robot_pos = self.robot_nodes[i].getPosition()
            robot_rot = self.robot_nodes[i].getOrientation()
...
def _machine_result(label, env, step_count, fault_robot, failure_time_min):
    ...
    return (
        ...
        f"fault_robot={fault_robot} "
        f"failure_time_min={failure_time_min:.4f}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fault-robot", type=int, default=-1,
                        help="Index (0-3) of the robot that fails. -1 = no fault.")
    parser.add_argument("--failure-time-min", type=float, default=float('inf'),
                        help="Sim-minutes at which the fault-robot permanently stops.")
    parser.add_argument("--stop-on-completion", action="store_true")
    args = parser.parse_args()

    fault_robot      = args.fault_robot
    failure_time_min = args.failure_time_min

    print("=" * 65)
    print("EXPERIMENT 4b — Robot Fault Tolerance")
    print(f"  Arena: 5x5m | Tags: 64 | Robots: 4 | Time: {args.duration_sim_min} min")
    if fault_robot >= 0:
        print(f"  Faulty robot: R{fault_robot + 1} (index {fault_robot}) — stopped from t=0")
    else:
        print("  Fault: none (control run)")
    print("=" * 65)

    env = EpuckForagingSupervisor()
    env.fault_robot      = fault_robot
    env.failure_time_min = failure_time_min
    ...
    while True:
        ...
        if completion_reached and args.stop_on_completion:
            print(_machine_result("COMPLETION_RESULT", env, step_count, fault_robot, failure_time_min), flush=True)
        ...
        if step_count >= target_steps:
            ...
            print(_machine_result(label, env, step_count, fault_robot, failure_time_min), flush=True)
            print(_batch_result(env), flush=True)
            env.simulationQuit(0)
            break
