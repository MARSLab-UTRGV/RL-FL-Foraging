...
class EpuckForagingSupervisor(Supervisor, gym.Env):
    def __init__(self):
        ...
        # Per-episode stats (reset each episode, used for logging)
        self.ep_pickups  = 0
        self.ep_deposits = 0

        # Logging
        self.LOG_STEP_EVERY = 500          # detailed per-step log interval
    ...
    def step(self, action):
        ...
        # Decay pheromone — slowed to 0.9995 so signal persists most of an episode.
        # At 0.999: peak of 5.0 drops below 0.1 threshold in ~3900 steps (95% of ep).
        # At 0.9995: same peak lasts ~7800 steps (~2 full episodes), giving PPO
        # a longer window to learn the cluster→deposit→return→cluster loop.
            ...
            if not self.carrying_state[i]:
                ...
                for tag_node in self.tag_nodes:
                    ...
                    if dist < 0.15:
                        ...
                        # Mark pickup location on pheromone grid so other robots
                ...
                if not picked_up:
                    ...
                    # Multiplier raised to 15.0 (was 10.0) to make pheromone-following
                    # a stronger gradient than the exploration reward, so PPO clearly
    ...
    def _log_step(self, obs, action):
        """Write detailed per-step info to training_log.txt (file only, no console)."""
        ...
        lines = [
            ...
            f"phero_max={phero_max:.3f} active_cells={active_cells}"
        ]
        ...
        for ri in range(self.num_robots):
            ...
            lines.append(
                ...
                f"cluster={obs_i[14]:.0f} cd={obs_i[15]:.2f} ca={obs_i[16]:+.2f} | "
                f"phero F={obs_i[17]:.2f} L={obs_i[18]:.2f} R={obs_i[19]:.2f} | "
                f"wall={wall_d:.2f}"
            )
    ...
    def _log_episode_summary(self):
        """Print episode summary to console and append to training_log.txt."""
        ...
        phero_max    = float(self.pheromone_grid.max())
        active_cells = int((self.pheromone_grid > 0.1).sum())
        phero_total  = float(self.pheromone_grid.sum())
        ...
        header = (
            ...
            f"  Curriculum: {phase}\n"
            f"  Pheromone:  max={phero_max:.3f} | "
            f"active_cells={active_cells} | total={phero_total:.1f}"
        )
        ...
        for ri in range(self.num_robots):
            ...
            if wall_d < 0.35 or max(prox) > 0.55:
                mode = "WALL_ESC"
            ...
            robot_lines.append(
                ...
                f"base={d2base:.2f} | "
                f"cluster_known={1 if phero_max > 0.1 else 0} | "
                f"phero_local={phero_local:.3f} | "
                f"wall={wall_d:.2f}"
            )
    ...
    def reset(self, seed=None, options=None):
        # Log the episode that just finished (skip the very first call at init)
...
if __name__ == "__main__":
    ...
    device = 'cpu'  # MLP policy with small batches is faster on CPU (no GPU transfer overhead)
