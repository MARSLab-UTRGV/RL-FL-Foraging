...
def read_percentages(csv_name: str) -> list[float]:
    frame = pd.read_csv(ROOT / csv_name)
    if "pct_collected" in frame.columns:
        percentages = frame["pct_collected"].dropna().tolist()
    else:
        required_columns = {"deposits", "num_tags"}
        missing_columns = required_columns - set(frame.columns)
        if missing_columns:
            missing = ", ".join(sorted(missing_columns))
            raise ValueError(f"{csv_name} is missing required columns: {missing}")
        percentages = (frame["deposits"] / frame["num_tags"] * 100).dropna().tolist()
...
def main() -> None:
    ...
    for swarm_index, (swarm, _) in enumerate(SWARMS):
        ...
        for model_index, (_, color, files_by_swarm) in enumerate(MODELS):
            data.append(read_percentages(files_by_swarm[swarm]))
    ...
    for group_index in range(len(SWARMS)):
    ...
    fig.savefig(ROOT / "exp2_percentage_boxplot_notched.png", bbox_inches="tight", dpi=600)
    fig.savefig(ROOT / "exp2_percentage_boxplot_notched.pdf", bbox_inches="tight")
    fig.savefig(ROOT / "exp2_percentage_boxplot_notched.eps", bbox_inches="tight")
