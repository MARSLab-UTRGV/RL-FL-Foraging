#!/usr/bin/env python3
"""Run 5x5 foraging completion-time evaluation batches."""
...
CONTROLLERS = {
    "centralized_ppo": "controllers/eval_best_model/eval_best_model_5x5.py",
    "cpfa_baseline": "controllers/cpfa_baseline/cpfa_baseline.py",
}
...
RESULT_RE = re.compile(
    ...
    r"pickups=(\d+) deposits=(\d+) steps=(\d+) elapsed_min=([-+0-9.eE]+)"
)
...
def run_sample(args, sample, port, logs_dir):
    ...
    prefix = f"{args.method}_{args.distribution}_{ARENA}_sample{sample}_port{port}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Run 5x5 foraging completion-time experiments."
    )
